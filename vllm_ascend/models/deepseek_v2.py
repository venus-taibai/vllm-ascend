...
class CustomDeepseekV2MLP(nn.Module):
    ...
    def forward(self, x, reduce_results: bool = True) -> torch.Tensor:
...
class CustomDeepseekV2MoE(nn.Module):
    ...
    def forward(
        ...
        if self.n_shared_experts is not None:
            if self.torchair_graph_enabled:
                if envs_ascend.VLLM_ENABLE_MC2 and not is_prefill:
                    shared_output = self.shared_experts(hidden_states, reduce_results=True)
                else:
                    shared_output = self.shared_experts(hidden_states, reduce_results=False)
            else:
                shared_output = self.shared_experts(hidden_states, reduce_results=True)

        if self.tp_size > 1 and not self.fused_experts_allgather_ep_enabled:
            if envs_ascend.VLLM_ENABLE_MC2 and not is_prefill:
        ...
        if self.dp_size > 1 and self.torchair_graph_enabled and not is_prefill:
