...
class LLMDataDistConnectorV1(KVConnectorBase_V1):
    ...
    def wait_for_save(self):
        ...
        for _, request in enumerate(metadata.requests):
            ...
            if request.request_id.endswith("-mt1"):
                # The user has set the output length for this request to one. As
                # a result, this request does not require processing on the
                # decode node. To prevent memory leaks, the KV cache for this
                # request will not be saved.
                continue
