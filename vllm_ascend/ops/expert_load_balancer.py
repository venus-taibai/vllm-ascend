...
class ExpertLoadBalancer:
    ...
    def __init__(self, expert_map_path: Optional[str], global_expert_num: int):
        ...
        # Only can record expert load if the
        # `VLLM_EXPERT_DISTRIBUTION_RECORDER_DIR` is set. And always enable
        # expert load recording if torchair graph is enabled and this env is
        # set.
        self._recording = \
            envs.VLLM_EXPERT_DISTRIBUTION_RECORDER_DIR is not None and \
                self._torchair_graph_enabled
    ...
    def get_rank_placement_map(self, layer_id, rank_id):
        ...
        # NOTE: valid num in expert_map may not equal to rank_local_expert_num when num_redundant_experts > 0
